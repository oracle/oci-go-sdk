...
POWERSHELL_CODEGEN_LOCATION = "codegen"
# Template for include each module in pom.xml
MODULE_TEMPLATE = "<module>{name}</module>"
# Pom.xml template specific to PowerShell
POWERSHELL_POM_FILE_TEMPLATE = """<?xml version='1.0' encoding='UTF-8'?>
<project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://maven.apache.org/POM/4.0.0 http://maven.apache.org/maven-v4_0_0.xsd">
  <modelVersion>4.0.0</modelVersion>
  <parent>
    <groupId>com.oracle.oci.sdk</groupId>
    <artifactId>oci-powershell-codegen</artifactId>
    <version>{sdk_version}</version>
    <relativePath>..</relativePath>
  </parent>
  <artifactId>oci-powershell-{service_name}-codegen</artifactId>
  <name>Oracle Cloud Infrastructure SDK - {service_friendly_name} Service Codegen</name>
  <description>This project contains the code generation spec and configuration for the {service_friendly_name}</description>
  <properties>
...
PS_GITHUB_WHITELIST_TEMPLATE = """\n^{spec_name}/Cmdlets/.*$
^{spec_name}/OCI.PSModules.{spec_name}.csproj$
^{spec_name}/OCI.PSModules.{spec_name}.psd1$"""

# The powershell-specific pom.xml template parameters for XML parsing
POWERSHELL_SPEC_PARAMS_XML_PATH_DICT = {
...
PROJECT_ENTRY_TEMPLATE = """Project("{{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}}") = "OCI.PSModules.{module_name}", "{module_name}\\OCI.PSModules.{module_name}.csproj", "{{{guid}}}"
EndProject
"""
...
class PowerShellSpecUpdater(SpecUpdaterBase):
...
def add_or_update_spec(artifact_id=None, group_id=None, spec_name=None, relative_spec_path=None, endpoint=None, subdomain=None, version=None, spec_generation_type=None, generate_waiters=True, generate_paginators=True, pom_location=None, github_whitelist_location=None):
    ...
    if found:
        ...
        generate_child_codegen_pom(sdk_dir, service_name, module_name, service_friendly_name, sdk_version, group_id,
                                   artifact_id, version, relative_spec_path, spec_generation_type, generate_waiters,
                                   generate_paginators)

        add_child_codegen_module(sdk_dir, service_name)
        update_sln_file(sdk_dir, module_name, guid)
        add_spec_module_to_powershell_github_whitelist(module_name, github_whitelist_location, PS_GITHUB_WHITELIST_TEMPLATE)
...
def find_existing_module(sdk_dir, artifact_id):
    codegen_dir = os.path.join(sdk_dir, POWERSHELL_CODEGEN_LOCATION)
    ...
    for ldr_path in pom_files:
        ...
        if len(properties) > 0 and artifact_id == properties[0].text:
            ...
            m = re.match("oci-powershell-([^-]*)-codegen", codegen_artifact_id)
            if m:
                return m.group(1)
...
def gather_settings(sdk_dir, spec_name):
    ...
    file_name = os.path.join(sdk_dir, POWERSHELL_CODEGEN_LOCATION, spec_name, "pom.xml")
...
def generate_child_codegen_pom(sdk_dir, service_name, module_name, service_friendly_name, sdk_version, group_id, artifact_id, artifact_version, spec_path_relative_to_jar, spec_generation_type, generate_waiters, generate_paginators):
    dir_name = os.path.join(sdk_dir, POWERSHELL_CODEGEN_LOCATION, service_name)
    os.mkdir(dir_name)
    file_name = os.path.join(dir_name, "pom.xml")

    content = POWERSHELL_POM_FILE_TEMPLATE.format(
        group_id=group_id,
...
def add_spec_module_to_powershell_github_whitelist(spec_name, github_whitelist_location, github_whitelist_template):
    if github_whitelist_location and github_whitelist_template:
        with open(github_whitelist_location, 'a') as f:
            f.write(github_whitelist_template.format(spec_name=spec_name))


def add_child_codegen_module(sdk_dir, service_name):
    file_name = os.path.join(sdk_dir, POWERSHELL_CODEGEN_LOCATION, "pom.xml")
    pom = parse_pom(file_name)
...
def update_sln_file(sdk_dir, module_name, guid):
    ...
    sln_file = os.path.join(sdk_dir, "oci-powershell-modules.sln")
