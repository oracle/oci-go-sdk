...
def single_pom_file_add_or_update_spec(artifact_id=None, group_id=None, spec_name=None,
    ...
    if (pom.findall(xpath_for_spec_dependency_declaration, ns)):
        ...
        previous = gather_settings(pom, artifact_id)
        ...
        if relative_spec_path:
            updated_spec |= update_relative_spec_path(pom, artifact_id, relative_spec_path)

        if update_endpoint_function:
            was_ignored = False
            if endpoint:
                was_changed, was_ignored = update_endpoint_function(pom, artifact_id, endpoint)
                updated_spec |= was_changed
            elif subdomain:
                was_changed, was_ignored = update_endpoint_function(pom, artifact_id, 'https://{}.{{domain}}'.format(subdomain))
                updated_spec |= was_changed
        ...
        current = gather_settings(pom, artifact_id)
        ...
        print('Artifact {} does not exist in pom.xml. Adding it...'.format(artifact_id))
        ...
        generate_and_add_generate_section(pom, spec_name, artifact_id, relative_spec_path, spec_generation_type, regional_sub_service_overrides, non_regional_sub_service_overrides, generate_execution_template)
