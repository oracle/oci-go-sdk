...
def generate_child_codegen_pom(sdk_dir, module_name, service_friendly_name, sdk_version, group_id, artifact_id,
                               artifact_version, spec_path_relative_to_jar, spec_generation_type, generate_waiters,
                               generate_paginators):
...
def update_version_of_existing_spec(sdk_dir, spec_name, version):
    ...
    if not version_increased:
        ...
        comment = ET.Comment(
            CODEGEN_POM_FILE_ARTIFACTORY_VERSION_COMMENT_TEMPLATE.format(version, old_version, datetime.datetime.now()))
...
def update_subdomain(sdk_dir, spec_name, subdomain):
    ...
    if results:
        ...
        print(
            'Ignored update to subdomain/endpoint -- service had no setting for this in pom.xml file and is getting that information from spec')
        return False, True
...
def add_sdk_clients_metadata_for_graalvm(sdk_dir, module_name):
    sdk_automatic_feature_metadata_file_name = os.path.join(sdk_dir, "bmc-addons", "bmc-graalvm-addon", "src", "main",
                                                            "java", "com", "oracle", "bmc", "graalvm",
                                                            "SdkAutomaticFeatureMetadata.java")
    new_sdk_clients_metadata_classname_template = "    \"com.oracle.bmc.{}.SdkClientsMetadata\","
    existingSdkClientsMetadataClasses = []

    with open(sdk_automatic_feature_metadata_file_name) as metadata_file:
        for line in metadata_file:
            if line.strip().startswith("\"com.oracle.bmc"):
                if not line.strip().endswith(","):
                    line += ","
                existingSdkClientsMetadataClasses.append(line.replace("\n", ""))
    existingSdkClientsMetadataClasses.append(new_sdk_clients_metadata_classname_template.format(module_name))
    existingSdkClientsMetadataClasses.sort()
    allSdkClientsMetadataClasses = "\n".join(existingSdkClientsMetadataClasses)[:-1]

    with open(sdk_automatic_feature_metadata_file_name, 'w') as metadata_file:
        metadata_file.write(GRAALVM_METADATA_TEMPLATE % allSdkClientsMetadataClasses)
...
def add_or_update_spec(artifact_id=None, group_id=None, spec_name=None, relative_spec_path=None, endpoint=None,
                       subdomain=None, version=None, spec_generation_type=None, generate_waiters=True,
                       generate_paginators=True, pom_location=None, github_whitelist_location=None):
    ...
    if found:
        ...
        if spec_generation_type:
            print(
                'Note: --spec-generation-type is ignored for the Java SDK, since it is set in the bmc-codegen/pom.xml file for all modules')
        ...
        generate_child_codegen_pom(sdk_dir, module_name, service_friendly_name, sdk_version, group_id, artifact_id,
                                   version, relative_spec_path, spec_generation_type, generate_waiters,
                                   generate_paginators)
        ...
        add_dependency_for_shaded_fatjar(os.path.join(sdk_dir, "bmc-shaded", "bmc-shaded-internal-fatjar", "pom.xml"),
                                         module_name, FULL_DEPENDENCY_TEMPLATE)
        add_dependency_for_shaded_fatjar(os.path.join(sdk_dir, "bmc-shaded", "bmc-shaded-full", "pom.xml"), module_name,
                                         SHADED_DEPENDENCY_TEMPLATE)
        add_spec_module_to_github_whitelist(module_name, github_whitelist_location, '\n^bmc-{}/.*')
        add_sdk_clients_metadata_for_graalvm(sdk_dir, module_name)

    print('Success!')

    return AddOrUpdateSpecResult(
        updated=(not found) or changed != [],
...
@click.option('--spec-name',
              help='The name of the spec. This will be used (e.g. core, identity, objectstorage). This is also used as the module name (\'bmc-servicename\') and base package (\'com.oracle.bmc.servicename\'). Underscores are removed, everything is lower-cased.')
@click.option('--relative-spec-path',
              help='The relative path of the spec within the artifact (e.g. coreservices-api-spec-20160918-external.yaml)')
@click.option('--endpoint', help='The base endpoint for the service (e.g. https://iaas.{domain}/20160918)')
@click.option('--subdomain',
              help='The subdomain for the service (e.g. if the endpoint is https://iaas.{domain}/20160918), the subdomain is "iaas"')
...
@click.option('--pom-location', type=click.Path(exists=True), default=DEFAULT_PARENT_POM_LOCATION,
              help='Location of the pom.xml file in the root directory of the OCI Java SDK')
@click.option('--github-whitelist-location', type=click.Path(exists=True), default=DEFAULT_GITHUB_WHITELIST_LOCATION,
...
def add_or_update_spec_command(artifact_id, group_id, spec_name, relative_spec_path, endpoint, subdomain, version,
                               spec_generation_type, generate_waiters, generate_paginators, pom_location,
                               github_whitelist_location):
