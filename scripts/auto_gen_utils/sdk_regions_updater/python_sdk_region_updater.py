...
class PythonSDKRegionsUpdater:
    script_dir = os.path.dirname(os.path.realpath(__file__))
    regions_json_file_path = os.path.join("tests", "resources", "regions.json")
    regions_source_file_path = os.path.join("src", "oci", "regions_definitions.py")
    regions_template_path = os.path.join(script_dir, "templates", "python-sdk-regions.tpl")
    mapping_entry_template = '    \'{}\': \'{}\',\n'
    list_item_template = '    \'{}\',\n'

    def __init__(self, sdk_path):
        self.logger = logging.getLogger("OCI-PythonSDK-Regions-Updater")
        logging.basicConfig(level=logging.INFO)
        self.python_sdk_path = sdk_path

    def _update_regions_file(self, regions):
        regions_source_file_full_path = os.path.join(self.python_sdk_path, self.regions_source_file_path)
        processed_regions = []
        processed_realms = []
        short_name_regions = ''
            ...
            for region in regions_by_realm_number[number]:
                ...
                if region['regionIdentifier'] not in processed_regions:
                    ...
                    if 'regionKey' in region:
                        short_name_regions += self.mapping_entry_template.format(region['regionKey'].lower(), region['regionIdentifier'].lower())
                    region_realm_mappings += self.mapping_entry_template.format(region['regionIdentifier'].lower(), region['realmKey'].lower())
                    regions_definitions += self.list_item_template.format(region['regionIdentifier'].lower())
    ...
    def update_regions(self, new_regions):
        regions_json_file_path = os.path.join(self.python_sdk_path, self.regions_json_file_path)
