...
def get_changed_files(legacy_java_sdk_dir, pr_id, base_dir):
    should_clean_up = False
    if not base_dir:
        should_clean_up = not KEEP_TEMP_FILES
        base_dir = clone_target_branch(pr_id, "legacy-java-sdk")
        if not base_dir:
            return None

    dcmp = dircmp(legacy_java_sdk_dir, base_dir, ['RCS', 'CVS', 'tags', '.git', 'target',
        'pom.xml.versionsBackup', '.DS_Store', '.idea'])
    files = get_diff_files(dcmp)

    prefix = os.path.join(legacy_java_sdk_dir, "")
...
def get_project(dir, group_id, artifact_id):
    ...
    # "/folders/legacy-java-sdk/bmc-codegen/{project}-codegen/pom.xml" where {project}
...
parser.add_argument('--legacy-java-sdk-dir', required=False, help='Directory of the Legacy Java SDK')
...
if args.legacy_java_sdk_dir:
    legacy_java_sdk_dir = os.path.abspath(args.legacy_java_sdk_dir)
else:
    legacy_java_sdk_dir = os.getcwd()

if args.keep_temp_files:
    KEEP_TEMP_FILES = True

clirr_files = find_clirr_files(legacy_java_sdk_dir)
if not clirr_files:
...
if args.build_type == 'DEXREQ':
    ...
    if issue:
        ...
        project = get_project(os.path.join(legacy_java_sdk_dir, "bmc-codegen"),
                            group_id,
                            artifact_id)
    ...
    try:
        changed_files = get_changed_files(legacy_java_sdk_dir, pr_id, args.base_dir)
...
if messages_for_modules:
    text = "Clirr detected backward incompatibilities possibly related to this change, compared to the latest release version of the OCI Legacy Java SDK:\n\n"
    ...
    if not dry_run:
        if args.build_type == "DEXREQ":
            ...
            make_general_comment("SDK", "legacy-java-sdk", pr_id, text)
    else:
