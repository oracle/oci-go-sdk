...
FULL_VERSION=`cat ../oci-powershell-modules/version.txt`-preview$BUILD_ID
python ./5_mark_preview_tickets_done_post_merge.py --build-id $BUILD_ID --tool PowerShell --full-version $FULL_VERSION --allow-transition-overall-issue-to-done --build-conf-name Sdk_OciPowershellTools_BuildPreviewModules
