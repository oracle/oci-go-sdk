...
def simplify_stack_traces(log):
    ...
    while line_num < len(lines):
        ...
        should_skip = False
        for substring in START_SKIPPING_SUBSTRINGS:
...
ERROR_SUBSTRINGS = [
    ...
    "WARN org.eclipse.jetty.server.handler.ContextHandler.ROOT - unavailable"
]


def check_and_summarize_log_file(log):
    for e in ERROR_SUBSTRINGS:
        if e in log:
            summarized_log = ""
            lines = log.split("\n")
            line_num = 0
            while line_num < len(lines):
                ...
                if e in line:
                    ...
                    # Only collect the first example
                    break

            return simplify_stack_traces(summarized_log)
...
FAILURE = """
The OCI testing service does not appear to have started correctly.

{author_text}Please use the information below to diagnose the problem. More information is also available in the [TeamCity build log]({tc_link}).
...
#
parser = argparse.ArgumentParser(description='Report testing service failure using service.stderr.log file.')
...
parser.add_argument('--log-input', required=True, help="Path to the log file for diagnostic output")
...
log = check_and_summarize_log_file(log)
